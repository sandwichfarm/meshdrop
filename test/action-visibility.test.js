...
test("Local discovery leaves IP room when same-instance backend is unsupported", () => {
    resetUi();
    const fired = [];
    const originalFire = globalThis.Events.fire;
    globalThis.Events.fire = (type, detail = {}) => {
...
test("FIPS action remains visible but unavailable until status confirms the daemon is reachable", async () => {
    ...
    assert.match(buttons.get("fips-discovery").title, /Signaling only: no FIPS instance ICE bridge descriptor is configured/);
...
test("Pollen action shows zero badge while visible without known peers", () => {
    ...
    assert.match(button.title, /Signaling only: no Pollen instance ICE bridge descriptor is configured/);
});
...
test("Instance route action shows same-instance peer count while enabled", () => {
    ...
    controller._localDiscoverySupported = true;
    controller._routeSupported = true;
