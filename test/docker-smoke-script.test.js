...
const fipsStreamSmokePath = new URL("../scripts/fips-stream-smoke.mjs", import.meta.url);
const fipsMeshSmokePath = new URL("../scripts/fips-mesh-smoke.mjs", import.meta.url);
...
test("Docker smoke initiates browser transfer proof against the built container", () => {
    ...
    assert.match(packageJson, /"test:i2p-stream": "node scripts\/i2p-stream-smoke\.mjs"/);
    assert.equal(fs.existsSync(i2pStreamSmokePath), true);
    const i2pStreamSmoke = fs.readFileSync(i2pStreamSmokePath, "utf8");
    assert.match(i2pStreamSmoke, /Proof i2p-http-stream/);
    assert.match(i2pStreamSmoke, /i2pd/);
    assert.match(i2pStreamSmoke, /\[httpproxy\]/);
    ...
    assert.match(packageJson, /"test:fips-stream": "node scripts\/fips-stream-smoke\.mjs"/);
    assert.equal(fs.existsSync(fipsStreamSmokePath), true);
    const fipsStreamSmoke = fs.readFileSync(fipsStreamSmokePath, "utf8");
    assert.match(fipsStreamSmoke, /Proof fips-instance-relay-route/);
    assert.match(packageJson, /"test:fips-mesh": "node scripts\/fips-mesh-smoke\.mjs"/);
    assert.equal(fs.existsSync(fipsMeshSmokePath), true);
    const fipsMeshSmoke = fs.readFileSync(fipsMeshSmokePath, "utf8");
    assert.match(fipsMeshSmoke, /Proof \$\{topology\.proof\}/);
    assert.match(fipsMeshSmoke, /fips-shared-public-peer-route/);
    assert.match(fipsMeshSmoke, /fips-different-public-peers-route/);
    assert.match(fipsMeshSmoke, /assertNoDirectPeerConfigFiles/);
    assert.match(fipsMeshSmoke, /directABConfig=false/);
    assert.match(fipsMeshSmoke, /route=fips primitive=fips-http-stream/);
    assert.match(fipsMeshSmoke, /hashMatched=true fallback=false/);
    assert.match(fipsMeshSmoke, /fips0Bytes=/);
    assert.match(fipsMeshSmoke, /forwardedBytes=/);
