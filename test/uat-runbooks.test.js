...
test("target UAT runbooks cover shipped build surfaces without overclaiming", () => {
    ...
    assert.match(mobile, /Xcode build on macOS, iOS device picker\s+UAT, App Group entitlement provisioning, share-sheet device UAT/);
    ...
    assert.match(targetStatus, /\| iOS \| Source artifact transfer smoke, native-source wrapper artifact, Xcode project source scaffold, and share extension source scaffold exist; app package\/device UAT open \|/);
    assert.match(
        targetStatus,
    ...
    assert.match(targetStatus, /Xcode build on macOS, native iOS app package, App Group entitlement provisioning/);
