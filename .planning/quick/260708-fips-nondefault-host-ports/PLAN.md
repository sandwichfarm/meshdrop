...
Publish container FIPS transports on non-default host ports while keeping the in-container FIPS daemon bind addresses unchanged. Also print the local FIPS DNS URL in the federation status log.
...
4. Verify compose config, Docker smoke, focused federation test, broad tests, diff check, and changed-code slop.
