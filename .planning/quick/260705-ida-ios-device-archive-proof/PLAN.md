# iOS Device Archive Proof

## Goal

Advance the iOS target toward the signed/device package gap by adding a deterministic unsigned generic `iphoneos`
archive artifact and release/CI readback wiring.

## Scope

- Add an iOS device archive builder that packages `MeshDrop.xcarchive` from generated native-source Xcode output.
- Add a smoke/test path that proves the archive shape and metadata without requiring signing credentials.
- Run the smoke from the existing macOS iOS CI lane.
- Publish and verify the archive as a release artifact.
- Update UAT ledgers to keep signed IPA, provisioning, device install, picker, share sheet, and native transfer UAT open.

## Non-goals

- Do not claim a signed or device-installable IPA.
- Do not claim App Store/TestFlight packaging.
- Do not claim App Group provisioning or physical iOS device UAT.

## Verification

- `node --check scripts/build-ios-device-archive-package.mjs`
- `node --check scripts/ios-device-archive-package-smoke.mjs`
- Focused tests for archive builder, CI workflow, release workflow, and UAT docs.
- Full repo gates before commit.
