...
| 2026-07-06 | `260706-n77-hide-the-legacy-relay-webrtc-header-togg` | complete | Visible header controls are round icon-only: Identity Nostr, Network Instance/FIPS/Pollen, Storage Blossom/Hashtree; no group words/protocol words; legacy relay anchor is outside header and not visible; hidden Relay discovery autostarts without restoring the visible switch and keeps stored follow recipients when relay contact-list lookup is missing; Pollen badge shows `0`; `npm test` 248/248; e2e includes Nostr/WebRTC transfer proof; Docker smoke includes two-host Nostr WebRTC transfer; localhost:3000 Docker image `sha256:f281aabe43ef83085383a6bac9c770214724f9fa2cd0da8b289aad597d1a19c7`; Playwright proves hidden relay active, public relay socket observed, no header overflow; changed-code slop clean; full-repo slop baseline remains policy-failing |
