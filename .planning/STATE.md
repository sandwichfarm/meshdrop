...
| 2026-07-06 | `260706-n77-hide-the-legacy-relay-webrtc-header-togg` | complete | Visible header controls are round icon-only: Identity Nostr, Network Instance/FIPS/Pollen, Storage Blossom/Hashtree; no group words/protocol words; legacy relay anchor is outside header and not visible; hidden Relay discovery autostarts without restoring the visible switch; Pollen badge shows `0`; `npm test` 246/246; e2e includes Nostr/WebRTC transfer proof; localhost:3000 Docker image `sha256:1df48feba18128baf72c61c140fe96b9e7d3b1e8ddf4df6635f97182ebcccea0`; Playwright proves hidden relay active, footer Relay badge visible, no header overflow; changed-code slop clean; full-repo slop baseline remains policy-failing |
