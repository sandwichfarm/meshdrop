...
9. For the native Linux archive, confirm `webrtc` and `nostr` are `false` until a native engine exposes
...
native runtime capability metadata, GTK/WebKit WebRTC gating, and a real Nostr WebRTC transfer between two browser peers
...
- The GTK/WebKit native shell does not expose `RTCPeerConnection` in current UAT, so native desktop WebRTC transfer UAT
  remains open.
- This artifact does not prove a signed installer.
