...
|--------|--------|---------------|---------------------------------|
| SPA | Chromium/Firefox backend-free transfer smoke exists; WebKit runtime smoke exists; Chromium public relay UAT exists | `SPA browser matrix` CI job; `npm run test:spa-artifact`; Chromium public relay run against `wss://bucket.coracle.social` | Firefox public relay UAT readback; WebKit transfer UAT |
